...
#define MY_ALLOC(T, p, size, allocFunc) { if ((size) == 0) p = 0; else if ((p = (T *)allocFunc((size) * sizeof(T))) == 0) return SZE_OUTOFMEMORY; }
...
SZ_RESULT SzArDbExFill (CArchiveDatabaseEx *db, void * (*allocFunc)(size_t size))
{
 ...
 for (i=0;i<db->Database.NumFiles;i++)
 {
   ...
   */
   while (1)
   {
    if (folderIndex >= db->Database.NumFolders) return SZE_ARCHIVE_ERROR;

    db->FolderStartFileIndex[folderIndex] = i;
    if (db->Database.Folders[folderIndex].NumUnPackStreams) break;
...
{
 return db->ArchiveInfo.DataStartPosition + db->PackStreamStartPositions[db->FolderStartPackStreamIndex[folderIndex] + indexInFolder];
}
...
CFileSize SzArDbGetFolderFullPackSize (CArchiveDatabaseEx *db, UInt32 folderIndex)
{
 ...
 for (i=0;i<folder->NumPackStreams;i++) size += db->Database.PackSizes[packStreamIndex + i];
 return size;
}
...
SZ_RESULT SafeReadDirect (ISzInStream *inStream, Byte *data, size_t size)
{
 ...
 while (size > 0)
 {
  ...
  if ((processedSize == 0) || (processedSize > size)) return SZE_FAIL;
...
int TestSignatureCandidate (Byte *testBytes)
{
 ...
 {
  if (testBytes[i] != k7zSignature[i]) return 0;
 }
...
{
 if (sd->Size == 0) return SZE_ARCHIVE_ERROR;
...
SZ_RESULT SzReadNumber32 (CSzData *sd, UInt32 *value)
{
 ...
 if (value64 >= 0x80000000) return SZE_NOTIMPL;
 if (value64 >= ((UInt64)(1) << ((sizeof(size_t) - 1) * 8 + 2))) return SZE_NOTIMPL;
...
{
 if (size > sd->Size) return SZE_ARCHIVE_ERROR;
...
SZ_RESULT SzWaitAttribute (CSzData *sd, UInt64 attribute)
{
 ...
 while(1)
 {
  ...
  if (type == attribute) return SZ_OK;
  if (type == k7zIdEnd) return SZE_ARCHIVE_ERROR;
...
SZ_RESULT SzReadBoolVector2 (CSzData *sd, size_t numItems, Byte **v, void * (*allocFunc)(size_t size))
{
 ...
 if (allAreDefined == 0) return SzReadBoolVector(sd, numItems, v, allocFunc);

 MY_ALLOC(Byte, *v, numItems, allocFunc);
 for (i=0;i<numItems;i++) (*v)[i] = 1;

 return SZ_OK;
}

SZ_RESULT SzReadHashDigests (CSzData *sd, size_t numItems, Byte **digestsDefined, UInt32 **digests, void * (*allocFunc)(size_t size))
{
 size_t i;
...
SZ_RESULT SzReadPackInfo (CSzData *sd, CFileSize *dataOffset, UInt32 *numPackStreams, CFileSize **packSizes, Byte **packCRCsDefined, UInt32 **packCRCs, void * (*allocFunc)(size_t size))
{
 UInt32 i;
 UInt64 type;
...
SZ_RESULT SzGetNextFolderItem (CSzData *sd, CFolder *folder, void * (*allocFunc)(size_t size))
{
 ...
 for (i=0;i<numCoders;i++) SzCoderInfoInit(folder->Coders + i);
 ...
 for (i=0;i<numCoders;i++)
 {
  CCoderInfo *coder = folder->Coders + i;
  {
   ...
   if ((mainByte & 0x20) != 0)
   {
    ...
    if (!SzByteBufferCreate(&coder->Properties, (size_t)propertiesSize, allocFunc)) return SZE_OUTOFMEMORY;
...
SZ_RESULT SzReadUnPackInfo (CSzData *sd, UInt32 *numFolders, CFolder **folders, /* for allocFunc */ void * (*allocFunc)(size_t size), ISzAlloc *allocTemp)
{
 UInt32 i, j;
 UInt64 type;
 SZ_RESULT res;
 ...
 for (i=0;i<*numFolders;i++) SzFolderInit((*folders) + i);
...
SZ_RESULT SzReadSubStreamsInfo (CSzData *sd, UInt32 numFolders, CFolder *folders, UInt32 *numUnPackStreams, CFileSize **unPackSizes, Byte **digestsDefined, UInt32 **digests, ISzAlloc *allocTemp)
{
 UInt64 type = 0;
 UInt32 i, j;
 UInt32 si = 0;
 UInt32 numDigests = 0;
 UInt32 numStreams;
 CFileSize sum = 0, size;
 int digestIndex = 0;
 Byte *digestsDefined2 = 0; 
 UInt32 *digests2 = 0;


 for (i=0;i<numFolders;i++) folders[i].NumUnPackStreams = 1;
 *numUnPackStreams = numFolders;
 ...
 while (1)
 {
  ...
  if ((type == k7zIdCRC) || (type == k7zIdSize) || (type == k7zIdEnd)) break;
...
SZ_RESULT SzReadStreamsInfo (CSzData *sd, CFileSize *dataOffset, CArchiveDatabase *db, UInt32 *numUnPackStreams, CFileSize **unPackSizes, /* allocTemp */ Byte **digestsDefined, /* allocTemp */ UInt32 **digests, /* allocTemp */ void * (*allocFunc)(size_t size), ISzAlloc *allocTemp)
{
 UInt64 type;
 ...
 while (1)
 {
  ...
  switch ((int) type)
  {
   ...
   case k7zIdSubStreamsInfo :
    RINOK(SzReadSubStreamsInfo(sd, db->NumFolders, db->Folders, numUnPackStreams, unPackSizes, digestsDefined, digests, allocTemp));
   break;
...
SZ_RESULT SzReadFileNames (CSzData *sd, UInt32 numFiles, CFileItem *files, void * (*allocFunc)(size_t size))
{
 ...
 for (i=0;i<numFiles;i++)
 {
  ...
  while (pos + 2 <= sd->Size)
  {
   value = (UInt32) (sd->Data[pos] | (((UInt32) sd->Data[pos + 1]) << 8));
   pos += 2;
   len++;
   ...
   {
   // if (value >= 0xDC00) return SZE_ARCHIVE_ERROR;		// bug here with somes archive ? (value = 0xDD4D)
   // if (pos + 2 > sd->Size) return SZE_ARCHIVE_ERROR;

    c2 = (UInt32) (sd->Data[pos] | (((UInt32) sd->Data[pos + 1]) << 8));
    pos += 2;

   // if ((c2 < 0xDC00) || (c2 >= 0xE000)) return SZE_ARCHIVE_ERROR;
    value = ((value - 0xD800) << 10) | (c2 - 0xDC00);
   }

   for (numAdds=1;numAdds<5;numAdds++)
   {
    if (value < (((UInt32) 1) << (numAdds * 5 + 6))) break;
   }
  ...
  while (2 <= sd->Size)
  {
   value = (UInt32) (sd->Data[0] | (((UInt32) sd->Data[1]) << 8));
...
SZ_RESULT SzReadHeader2 (CSzData *sd, CArchiveDatabaseEx *db, /* allocMain */ CFileSize **unPackSizes, /* allocTemp */ Byte **digestsDefined, /* allocTemp */ UInt32 **digests, /* allocTemp */ Byte **emptyStreamVector, /* allocTemp */ Byte **emptyFileVector, /* allocTemp */ ISzAlloc *allocMain, ISzAlloc *allocTemp)
{
 UInt64 type, size;
 UInt32 i, numUnPackStreams = 0;
 UInt32 numFiles = 0;
 CFileItem *files = 0;
 ...
 {
  RINOK(SzReadStreamsInfo(sd, &db->ArchiveInfo.DataStartPosition, &db->Database, &numUnPackStreams, unPackSizes, digestsDefined, digests, allocMain->Alloc, allocTemp));
 ...
 if (type == k7zIdEnd) return SZ_OK;
 if (type != k7zIdFilesInfo) return SZE_ARCHIVE_ERROR;
 ...
 for (i=0;i<numFiles;i++) SzFileInit(files + i);
  ...
  else
  {
   switch ((int) type)
   {
    ...
    case k7zIdEmptyStream:
     ...
     for (i=0;i<numFiles;i++)
	 {
      if ((*emptyStreamVector)[i]) numEmptyStreams++;
	 }
    break;
...
SZ_RESULT SzReadHeader (CSzData *sd, CArchiveDatabaseEx *db, ISzAlloc *allocMain, ISzAlloc *allocTemp)
{
 ...
 SZ_RESULT res = SzReadHeader2(sd, db, &unPackSizes, &digestsDefined, &digests, &emptyStreamVector, &emptyFileVector, allocMain, allocTemp);
...
SZ_RESULT SzReadAndDecodePackedStreams2 (ISzInStream *inStream, CSzData *sd, CSzByteBuffer *outBuffer, CFileSize baseOffset, CArchiveDatabase *db, CFileSize **unPackSizes, Byte **digestsDefined, UInt32 **digests, ISzAlloc *allocTemp)
{
 ...
 RINOK(SzReadStreamsInfo(sd, &dataStartPos, db, &numUnPackStreams,  unPackSizes, digestsDefined, digests, allocTemp->Alloc, allocTemp));
 dataStartPos += baseOffset;

 if (db->NumFolders != 1) return SZE_ARCHIVE_ERROR;

 folder = db->Folders;
 unPackSize = SzFolderGetUnPackSize(folder);

 RINOK(inStream->Seek(inStream, dataStartPos));
 if (!(SzByteBufferCreate(outBuffer, (size_t) unPackSize, allocTemp->Alloc))) return SZE_OUTOFMEMORY;

 res = SzDecode(db->PackSizes, folder, inStream, outBuffer->Items, (size_t) unPackSize, &outRealSize, allocTemp);
 RINOK(res);

 if (outRealSize != (UInt32) unPackSize) return SZE_FAIL;

 if (folder->UnPackCRCDefined)
 {
  if (!(CrcVerifyDigest(folder->UnPackCRC, outBuffer->Items, (size_t)unPackSize))) return SZE_FAIL;
 }

 return SZ_OK;
}

SZ_RESULT SzReadAndDecodePackedStreams (ISzInStream *inStream, CSzData *sd, CSzByteBuffer *outBuffer, CFileSize baseOffset, ISzAlloc *allocTemp)
{
 CArchiveDatabase db;
 CFileSize *unPackSizes = 0;
 Byte *digestsDefined = 0;
 ...
 res = SzReadAndDecodePackedStreams2(inStream, sd, outBuffer, baseOffset, &db, &unPackSizes, &digestsDefined, &digests, allocTemp);
...
SZ_RESULT SzArchiveOpen2 (ISzInStream *inStream, CArchiveDatabaseEx *db, ISzAlloc *allocMain, ISzAlloc *allocTemp)
{
 ...
 UInt32 crc;
 CFileSize pos = 0;
 CSzByteBuffer buffer, outBuffer;
 CSzData sd;
 SZ_RESULT res;
 UInt64 type;
 ...
 if (!(TestSignatureCandidate(signature))) return SZE_ARCHIVE_ERROR;

 RINOK(SafeReadDirectByte(inStream, &version));
 if (version != k7zMajorVersion) return SZE_ARCHIVE_ERROR;
 ...
 if (CrcGetDigest(&crc) != crcFromArchive) return SZE_ARCHIVE_ERROR;
 if (nextHeaderSize == 0) return SZ_OK;

 RINOK(inStream->Seek(inStream, (CFileSize)(pos + nextHeaderOffset)));
 if (!(SzByteBufferCreate(&buffer, (size_t) nextHeaderSize, allocTemp->Alloc))) return SZE_OUTOFMEMORY;
 ...
 if (res == SZ_OK)
 {
  if (CrcVerifyDigest(nextHeaderCRC, buffer.Items, (UInt32) nextHeaderSize))
  {
   while (1)
   {
	...
	res = SzReadAndDecodePackedStreams(inStream, &sd, &outBuffer, db->ArchiveInfo.StartPositionAfterHeader, allocTemp);
    if (res != SZ_OK)
    {
...
SZ_RESULT SzArchiveOpen (ISzInStream *inStream, CArchiveDatabaseEx *db, ISzAlloc *allocMain, ISzAlloc *allocTemp)
{
 SZ_RESULT res = SzArchiveOpen2(inStream, db, allocMain, allocTemp);

 if (res != SZ_OK) SzArDbExFree(db, allocMain->Free);
